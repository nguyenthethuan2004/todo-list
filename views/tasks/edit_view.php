...
<div class="edit-grid">
    <div class="left-stack">
        <div class="card shadow-sm edit-main">
            <div class="card-body">
                ...
                <form method="POST" action="<?= url('app/controllers/taskController.php?action=update&id=' . (int)$formData['id']) ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control description-field" rows="4"><?= sanitize($formData['description']) ?></textarea>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
        ...
        <?php if (!empty($isOwner)): ?>
            <div class="card shadow-sm share-card">
                <div class="card-body">
                    <h4 class="mb-3">Chia sẻ & quyền</h4>
                    <form class="share-form d-flex flex-wrap align-items-center gap-2 mb-3" method="POST" action="<?= url('app/controllers/shareController.php?action=add&task_id=' . (int)$formData['id']) ?>">
                        <input type="hidden" name="redirect" value="<?= $redirect ?>">
                        <input type="text" name="username" class="form-control share-username" placeholder="Username">
                        <select name="role" class="form-select share-role">
                            <option value="viewer">Viewer</option>
                            <option value="editor">Editor</option>
                        </select>
                        <button class="btn btn-outline-primary share-submit text-nowrap">Thêm</button>
                    </form>
    ...
    <div class="side-stack">
        ...
        <div class="card-body">
            ...
            <form class="d-flex gap-2 mb-3 flex-column flex-sm-row" method="POST" action="<?= url('app/controllers/subtaskController.php?action=create&task_id=' . (int)$formData['id']) ?>">
                <input type="hidden" name="redirect" value="<?= $redirect ?>">
                <input type="text" name="title" class="form-control" placeholder="Thêm mục checklist">
                <button class="btn btn-outline-primary text-nowrap">Thêm</button>
            </form>
    ...
    <div class="card shadow-sm side-card">
        <div class="card-body">
            ...
            <form class="row g-2 align-items-center mb-3" method="POST" action="<?= url('app/controllers/reminderController.php?action=create&task_id=' . (int)$formData['id']) ?>">
                <input type="hidden" name="redirect" value="<?= $redirect ?>">
                ...
                <div class="col-12 col-md-2">
                    <button class="btn btn-primary w-100 text-nowrap">Lưu</button>
                </div>
            </form>
    ...
    <div class="card shadow-sm side-card">
        <div class="card-body">
            ...
            <form class="d-flex gap-2 mb-3 attachment-upload flex-column flex-sm-row" method="POST" enctype="multipart/form-data" action="<?= url('app/controllers/attachmentController.php?action=upload&task_id=' . (int)$formData['id']) ?>">
                <input type="hidden" name="redirect" value="<?= $redirect ?>">
                <input type="file" name="file" class="form-control" required>
                <button class="btn btn-outline-primary text-nowrap">Tải lên</button>
            </form>
    ...
    <div class="card shadow-sm side-card">
        <div class="card-body">
            ...
            <form class="mb-3" method="POST" action="<?= url('app/controllers/commentController.php?action=create&task_id=' . (int)$formData['id']) ?>">
                <input type="hidden" name="redirect" value="<?= $redirect ?>">
                <textarea name="content" rows="2" class="form-control comment-field" placeholder="Nhập bình luận..."></textarea>
